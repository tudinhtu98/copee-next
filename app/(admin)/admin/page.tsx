...
export default function AdminHome() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const res = await fetch("/api/proxy/admin/summary", {
          cache: "no-store",
        });
        if (!res.ok) {
          const data = await res
            .json()
            .catch(() => ({ message: "Không thể tải thống kê" }));
  ...
  return (
    <div className="space-y-3">
      ...
      {error && (
        <div className="rounded-md border border-destructive text-destructive px-3 py-2 text-sm">
          {error}
        </div>
      )}
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <div className="rounded-md border p-4">
          Số user: {summary ? summary.users : "..."}
        </div>
        <div className="rounded-md border p-4">
          Tiền user đã dùng:{" "}
          {summary
