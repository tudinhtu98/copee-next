...
export default function SettingsPage(){
  const [form, setForm] = useState({ name: '', baseUrl: '', wooConsumerKey: '', wooConsumerSecret: '' })
  ...
  async function onSave(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setForm({ name: '', baseUrl: '', wooConsumerKey: '', wooConsumerSecret: '' })
      mutate()
    } finally {
      setIsSaving(false)
    }
  ...
  async function onRemoveSite(id: string) {
    ...
    try {
      ...
      if (!res.ok) {
        const data = await res.json().catch(() => ({ message: 'Không thể xoá site' }))
  ...
  return (<div className="space-y-3">
    ...
    <form className="grid gap-3 max-w-xl" onSubmit={onSave}>
      <Input placeholder="Tên site" value={form.name} onChange={(e)=>setForm((prev)=>({ ...prev, name: e.target.value }))} required />
      <Input placeholder="Base URL (https://...)" value={form.baseUrl} onChange={(e)=>setForm((prev)=>({ ...prev, baseUrl: e.target.value }))} required />
      <Input placeholder="Woo Consumer Key" value={form.wooConsumerKey} onChange={(e)=>setForm((prev)=>({ ...prev, wooConsumerKey: e.target.value }))} required />
      <Input placeholder="Woo Consumer Secret" value={form.wooConsumerSecret} onChange={(e)=>setForm((prev)=>({ ...prev, wooConsumerSecret: e.target.value }))} required />
    ...
    <div className="space-y-2">
      ...
      {error && <div className="text-sm text-destructive">Không thể tải danh sách site</div>}
      <div className="grid gap-2">
        {sites?.map((site) => (
          <div key={site.id} className="flex items-center justify-between rounded-md border p-3">
            ...
            <div className="flex gap-2">
              ...
              <Button variant="outline" size="sm" onClick={() => onRemoveSite(site.id)}>
                Xoá
              </Button>
            </div>
          </div>
        ...
        {sites && sites.length === 0 && !isLoading && (
          <div className="text-sm text-muted-foreground">Chưa có site nào</div>
        )}
      </div>
    </div>
  </div>)
